 ...
 */
function getGridCoords(event: MouseEvent, yOffset: number, ref: React.RefObject<SVGSVGElement>) {
...
function GridBlock({ x, y, ...props }: GridBlockProps) {
    const transformValue = `translate(${-32 * y + GRID_WIDTH * x} ${GRID_HEIGHT * y})`;
    return (
        <motion.path
...
export function GridPattern({ yOffset = 0, interactive = false, ...props }: GridPatternProps) {
     ...
     */
    function updateHoveredBlocks(blocks: number[][], adjustedX: number, adjustedY: number, key: number): number[][] {
        ...
        if (!updatedBlocks.some(block => block[0] === adjustedX && block[1] === adjustedY)) {
    ...
    useEffect(() => {
        ...
        function onMouseMove(event: MouseEvent) {
            const { adjustedX = 0, adjustedY = 0 } = getGridCoords(event, yOffset, ref);

            if (currentBlock.current?.[0] === adjustedX && currentBlock.current?.[1] === adjustedY) {
                return;
            }
            ...
            setHoveredBlocks(blocks => updateHoveredBlocks(blocks, adjustedX, adjustedY, key));
        }
    ...
    return (
        <motion.svg ref={ref} aria-hidden="true" {...props}>
            ...
            <motion.svg x="50%" y={yOffset} strokeWidth="0" className="overflow-visible">
                <GridBlock x={1} y={1} animate={{ opacity: [0, 1, 0] }} />

                {hoveredBlocks.map(block => (
                    <GridBlock
                        onAnimationComplete={() => { setHoveredBlocks(blocks => blocks.filter(b => b[2] !== block[2])) }}
